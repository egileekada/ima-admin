...
export default function NewListing(){
    ...
    return(
        <div>  
            ...
            //             return( 
            //                 <div key={item._id} className={styles.myPropsHolder}>
            //                     <UserInfo img={item?.imagesURLs[0]} property={item?.name} price={(item?.price).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")} 
            //                     listby="Ima Original" type={item?.type} date={new Date(item?.createdAt).toUTCString()} location={item?.location?.address+", "+item?.location?.city+", "+item?.location?.state}
            //                     status=''/>  
            //                 </div>
            //                 )
            //             })}
