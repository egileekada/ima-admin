...
export default function Index() {
    ...
    const submit = async ( ) => {  
        ...
        try {   
            const request: any = await axios.default.patch(`${BASEURL.URL}/auth/admin-change-password`, {   
    ...
    return (
        <div className=' w-full py-12 px-16 ' >
            ...
            <div style={{ width: "500px"}} className="mt-10 flex flex-col" > 
                ...
                <div className=' w-full relative mt-2 ' > 
                    <input onChange={(e)=> setNewPassWord(e.target.value)} type={showNewPassWord ? "text": "password"}  style={{ fontFamily: "Montserrat", fontWeight: "400", color: "#000"}} className=' w-full h-12 border px-6 border-[#D4D4D4] rounded-md ' placeholder='Enter password ' />
                ...
                <button onClick={submit} style={{ fontFamily: "Montserrat", fontWeight: "700", color: "#fff"}}  className=' w-64 rounded-md bg-[#0984D6] text-white mt-8 mx-auto h-12 ' >{loading ? 'loading..':'Save'}</button>
            </div>
        </div>
    )
}