...
export default function Index() {
    ...
    const submit = async ( ) => {  
        ...
        try { 
            ...
            imageFiles.map((items: any)=> {
                return(
                    formData.append('images', items) 
                )
            })    
    
            const request: any = await axios.default.post(`${BASEURL.URL}/settings/banner`, formData, {
            ...
            <div className=' w-full p-6 py-8 h-full border border-black rounded-md  ' >
                ...
                {tab === 0 && 
                    <div className=' w-fit ' > 
                        ...
                        <input className=' border border-black rounded-md w-[600px] h-12 py-2 px-4 mt-2 mb-6 '  type="file" multiple onChange={handleImageChange} />
                ...
                <div className='grid grid-cols-4 mb-4 pb-4 px-2 gap-4 flex-1 overflow-x-auto mt-5' > 
                            {!isLoading && (
                                <>
                                    {data?.data?.banner.map((item: any, index: any) =>{ 
                                        return(  
                                            <div key={index}  className=' w-44 mr-3 text-sm ' > 
                                                <div className=' w-44 h-32 mb-4 ' > 
                ...
                {tab === 2 && 
                    <div className=' w-fit ' > 
                        ...
                        {indexID && ( 
                            <button onClick={()=> setDeleteModal(true)} className=' w-36 h-11 mt-12 text-white rounded-md bg-[#FF333F]  '  style={{fontFamily: "Montserrat", fontWeight: "700"}} >Delete</button>
                        )}
                        {deleteModal ? 
                            (
                                <>
